...
public class ExameDao extends Dao {
	...
	public void excluirExame(Integer codigo) {
		StringBuilder query = new StringBuilder("DELETE FROM exame WHERE cd_exame = ?");
		try (Connection con = getConexao();
				PreparedStatement ps = con.prepareStatement(query.toString())) {
	...
	public void editarExame(ExameVo exameVo) {
		StringBuilder query = new StringBuilder("UPDATE exame SET (nm_exame) = ? WHERE cd_exame = ?");
		try (Connection con = getConexao();
				PreparedStatement ps = con.prepareStatement(query.toString())) {

			int i = 1;
			ps.setString(i++, exameVo.getNome());
			ps.setInt(i++, Integer.parseInt(exameVo.getCdExame()));
			ps.executeUpdate();
	...
	public List<ExameVo> findAllExames() {
		StringBuilder query = new StringBuilder("SELECT cd_exame id, nm_exame nome FROM exame");
		try (Connection con = getConexao();
				PreparedStatement ps = con.prepareStatement(query.toString());
