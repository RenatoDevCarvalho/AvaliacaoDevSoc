...
public abstract class Dao implements AutoCloseable {
    ...
    private void conectar() {
        ...
        if (primeiraInicializacao) {
            urlBuilder.append(
                    "INIT=runscript from 'classpath:CRIA_TABELAS_E_INSERE_REGISTROS_INICIAIS.sql';"
            );
        ...
        try {
            ...
            throw new TechnicalException(
                    "Ocorreu um problema na tentativa de conexao",
                    ex
            );
    ...
    private void fechar() throws SQLException {
        if (con == null) throw new TechnicalException("Conexao nao foi criada");

        if (con.isClosed()) throw new TechnicalException(
                "Conexao ja foi encerrada"
        );
    ...
    protected Connection getConexao() throws SQLException {
        if (con == null || con.isClosed()) conectar();
        return con;
    }
}
