...
<html>
	...
	<body class="bg-secondary">
		<nav class="navbar bg-light px-3 justify-content-start" style="background-color: #e3f2fd;">
					...
					<s:form action="/filtrarExames.action">
						<div class="input-group">
							<span class="input-group-text"> <strong>
								<s:text	name="label.buscar.por" /></strong>
							</span>
							<s:select
					...
					<tbody>
						<s:iterator value="exames">
							<tr>
								<td>${cdExame}</td>
								<td>${nome}</td>
								<td class="text-end"><s:url action="editarExame"
										var="editar">
										<s:param name="exameVo.cdExame" value="cdExame"></s:param>
									</s:url> <a href="${editar}" class="btn btn-warning text-white"> <s:text name="label.editar" />
								</a> <s:url action="excluirExame" var="excluir">
										<s:param name="exameVo.cdExame" value="cdExame"></s:param>
									</s:url> <a href="${excluir}" class="btn btn-danger"> <s:text name="label.excluir" />
								</a></td>
							</tr>
						</s:iterator>
					</tbody>
